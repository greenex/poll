...
/*
	...
	*/
	...
	function answer(pollId){
		
	alert($('input:radio[name=pollAnswerID_'+pollId+']:checked'));
		var pollAnswerVal = $('input:radio[name=pollAnswerID_'+pollId+']:checked').val();//Getting the value of a selected radio element.
		if ($('input:radio[name=pollAnswerID_'+pollId+']:checked').length) {
			$("#pollAjaxLoader_"+pollId).show(); //show the ajax loader
			$.ajax({  
				type: "POST",  
				url: "inc/functions.php",  
				data: { pollAnswerID: pollAnswerVal, action: "vote" },
				success: function(theResponse) { 
					...
					//the functions.php returns a response like "1|13|#ffcc00-2|32|#00ff00-3|18|#cc0000-63" which the first number is the answerID, second is the points it has and third is the color for that answer's graph. The last number is the sum of all points for easilt calculating percentages.
					if (theResponse == "voted") { 
						$("#pollAjaxLoader_"+pollId).hide(); //hide the ajax loader
						$("#pollMessage_"+pollId).html("sorry, you already voted.").fadeTo("slow", 1);
					} else {
						var numberOfAnswers 		= (theResponse).split("-").length-2;//calculate the number of answers
						var splittedResponse 		= (theResponse).split("-");
						var pollAnswerTotalPoints 	= splittedResponse[numberOfAnswers+1];
						...
						$("#pollAjaxLoader_"+pollId).hide(); //hide the ajax loader again
						$("#pollSubmit_"+pollId).attr("disabled", "disabled"); //disable the submit button
					}
				}  
			});  
			return false; 
			...
			$("#pollMessage_"+pollId).html("please select an answer.").fadeTo("slow", 1, function(){
				setTimeout(function() {
					$("#pollMessage").fadeOut("slow");
				}, 3000);																		 
			});
